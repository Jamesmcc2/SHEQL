...
{
	  	$query = "select * from sn_images where id='".$_REQUEST['del_id']."'";
		$pic=$qry->querySelectSingle($query);
		$sql="DELETE FROM  sn_images WHERE id='".$_REQUEST['del_id']."'";
...
<div class="col-md-2">
    ...
    <?
		...
		$queryData="Select *from sn_images where picture_id='".$pic_id."' ";
			...
			{?> <div class="thumbnail"> <img src="uploads/thumbs/<?=$prod['file'] ?>"><div align="center"><a onClick="return confirmAction();" href="?del_id=<?=$prod['id'] ?>&pic_id=<?=$pic_id ?>">Delete</a></div></div><? } ?>
